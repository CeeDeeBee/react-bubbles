...
- [x] Explain what a token is used for.
  ...
  Tokens are long alpha-numeric strings that are used to authenticate a client with a server. After a client has succesfully sent valid credentials the server will send back a corresponding token which acts like a key and allows the client to access protected api routes and private client-side routes.
...
- [x] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.
  ...
  A GET request is for retrieving data and corresponds to the Read portion of CRUD. A POST request is for creating data and is the C in CRUD. A PUT request is for updating data and is the U in the acronym. FInally, a delete request is for deleting data and is the D in CRUD.
